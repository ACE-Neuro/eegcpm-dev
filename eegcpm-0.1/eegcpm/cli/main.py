...
For more information: https://github.com/clivehywong/eegcpm-dev
    ...
    source_parser.add_argument(
        '--preprocessing',
        type=str,
        help='Preprocessing pipeline variant (e.g., optimized, standard). Falls back to config depends_on.preprocessing if omitted'
    )
    source_parser.add_argument(
        '--task',
        type=str,
        help='Task name (e.g., cptaud). Falls back to config depends_on.task if omitted'
    )
    source_parser.add_argument(
        '--subjects',
        type=str,
